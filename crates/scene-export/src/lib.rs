...
struct ReplayInfo {
    ...
    game_build: String,
    /// Short "major.minor" client version, e.g. "15.5" (from clientVersionFromExe).
    #[serde(skip_serializing_if = "Option::is_none")]
    game_version_short: Option<String>,
    /// Raw `matchGroup` from the replay meta (e.g. "pvp", "ranked", "clan");
    /// the web player maps it to a friendly battle-type label.
    #[serde(skip_serializing_if = "Option::is_none")]
    battle_type: Option<String>,
    /// Raw WoWS wall-clock `dateTime`, e.g. "16.02.2026 19:05:19".
    #[serde(skip_serializing_if = "Option::is_none")]
...
/// Decode a finalized `.wowsreplay` into the TFD battle-scene JSON, fully
/// self-contained (map image and Arms Race powerup icons inlined as data URLs).
...
pub fn export_scene_json(game_dir: &Path, replay_path: &Path) -> Result<String> {
    ...
    if let Some(build) = version.build_number() {
        let translations = game_data::translations_path(game_dir, build);
    ...
    // The scene is always self-contained: the map is inlined as a data URL so
    // the bridge serves the entire scene from a single response.
    let map_file = match load_map_image(&replay.meta.mapName, &vfs) {
        Some(image) => {
    ...
    // Completeness uses the SAME BattleResults signal as the picker list (see
    ...
    let scene = ReplayScene {
        ...
        replay: ReplayInfo {
            ...
            game_version_short: short_game_version(&replay.meta.clientVersionFromExe),
...
pub struct ReplayMeta {
    /// Raw `matchGroup` (e.g. "pvp", "ranked", "clan"); None on very old clients.
    pub battle_type: Option<String>,
    /// Short "major.minor" client version, e.g. "15.5".
    pub game_version_short: Option<String>,
    /// WoWS map identifier (e.g. "spaces/13_OC_new_dawn").
    pub map_name: String,
    /// Raw WoWS wall-clock `dateTime`, e.g. "16.02.2026 19:05:19".
    pub date_time: String,
    /// Recording player's in-game name.
    pub player_name: String,
    /// Recording player's ship index code (e.g. "PVSB010").
...
/// files. Decrypts + decompresses the packet stream and walks packet *headers*
/// only (`RawPacketIterator`, no entity specs), so it works for every replay
/// including old builds the installed game can't fully decode. Used to enrich
/// the player's replay picker (battle type, version, complete/incomplete).
...
/// packet is present. Absent means the recording stopped early — the player
/// left before the battle finished. Walks packet *headers* only
/// (`RawPacketIterator`, no entity specs / game files / payload decode), so it
/// is cheap and works for any client build. The scene decode and the picker
/// list both use this ONE signal so a replay is never "complete" in one place
/// and "incomplete" in the other.
fn recording_complete(packet_data: &[u8], client_version_from_exe: &str) -> bool {
    let version = Version::from_client_exe(client_version_from_exe);
    RawPacketIterator::with_version(packet_data, version)
        .filter_map(Result::ok)
        .any(|packet| matches!(packet.packet_type, PacketTypeId::BattleResults))
}

/// "15,5,0,11966999" (WoWS `clientVersionFromExe`) → "15.5". Returns None when
/// the string has no two leading numeric components.
fn short_game_version(client_version_from_exe: &str) -> Option<String> {
    let mut parts = client_version_from_exe
        .split([',', '.'])
        .map(str::trim)
...
/// Encode the Arms Race powerup icons this battle actually used as inline
/// `data:image/png;base64,...` URLs, keeping the scene JSON self-contained.
fn powerup_icons_inline(
    samples: &[BuffSample],
